...
class LoginService extends BaseService
{
    ...
    public function login(array $params): array
    {
        ...
        if (empty($user)) {
            $this->throwExp(StatusCode::ERR_USER_ABSENT, '登录失败，用户不存在');
        }
        if (md5($params['password']) != $user->password) {
            $this->throwExp(StatusCode::ERR_USER_PASSWORD, '登录失败，用户验证失败，密码错误');
        }
        if ($user['status'] != 1) {
            $this->throwExp(StatusCode::ERR_USER_DISABLE, '该账户已经被停用，请联系管理员');
        }
    ...
    public function register(array $params): bool
    {
        ...
        if (!env('APP_TEST')) {
            ...
            $redis = $container->get(\Hyperf\Redis\Redis::class);
            $code = $redis->get($params['code_key']);
            if (strtolower($params['captcha']) != strtolower($code)) {
                $this->throwExp(StatusCode::ERR_CODE, '验证失败，验证码错误');
            }
        }
        ...
        if (!$user->save()) {
            $this->throwExp(StatusCode::ERR_EXCEPTION, '注册用户失败');
        }
    ...
    public function getRouters(): array
    {
        ...
        foreach ($permissionList as $key => $val) {
            if ($val['status'] == Permission::OFF_STATUS) {
                unset($permissionList[$key]);
            }
            if ($val['type'] == Permission::BUTTON_OR_API_TYPE) {
                unset($permissionList[$key]);
            }
        }
