...
class UserController extends AbstractController
{
    ...
    public function index()
    {
        ...
        if (!empty($this->request->input('username'))) {
            $userQuery->where('username', 'like', '%' . $this->request->input('username') . '%');
        }
        if (!empty($this->request->input('desc'))) {
            $userQuery->where('desc', 'like', '%' . $this->request->input('desc') . '%');
        }
        if (strlen($status)) {
    ...
    public function store()
    {
        ...
        $user->avatar = empty($postData['avatar']) ? 'http://landlord-res.oss-cn-shenzhen.aliyuncs.com/admin_face/face' . rand(1,
                10) . '.png' : $postData['avatar'];
        ...
        if (!$user->save()) {
            $this->throwExp(StatusCode::ERR_EXCEPTION, '添加用户失败');
        }
    ...
    public function edit(int $id)
    {
        ...
        if (empty($userInfo)) {
            $this->throwExp(StatusCode::ERR_USER_ABSENT, '获取用户信息失败');
        }
    ...
    public function profileEdit($id)
    {
        ...
        $user->address = $postData['address'] ?? '';
        if (!$user->save()) {
            $this->throwExp(StatusCode::ERR_EXCEPTION, '修改用户信息失败');
        }
    ...
    public function uploadAvatar()
    {
        ...
        if ($params['file']->getSize() > 30000000) {
            $this->throwExp(UploadCode::ERR_UPLOAD_SIZE, '上传图片尺寸过大');
        }
        ...
        $user->save();
        if (!$user->save()) {
            $this->throwExp(StatusCode::ERR_VALIDATION, '修改用户头像失败');
        }
        
        return $this->success([
            'fileName' => $fileName,
            'url'      => config('url') . '/' . $fileUrl
        ], '上传图片成功');
    }
    ...
    public function update(int $id)
    {
        ...
        $user = User::getOneByUid($id);
        $user->status = $postData['status'] ?? '1';
        $user->avatar = $postData['avatar'] ?? 'http://landlord-res.oss-cn-shenzhen.aliyuncs.com/admin_face/face' . rand(1,
                10) . '.png';
        $user->desc = $postData['desc'] ?? '';
        $user->mobile = $postData['mobile'] ?? '';
        $user->sex = $postData['sex'] ?? '';
    ...
    public function resetPassword()
    {
        ...
        if (empty($userInfo)) {
            $this->throwExp(400, '账号不存在');
        }
        if (md5($params['new_password']) != md5($params['confirm_password'])) {
            $this->throwExp(StatusCode::ERR_EXCEPTION, '两次密码输入不一致');
        }
        if (!empty($params['old_password']) && md5($params['old_password']) != $userInfo['password']) {
            $this->throwExp(StatusCode::ERR_EXCEPTION, '旧密码验证失败，请重试');
        }
